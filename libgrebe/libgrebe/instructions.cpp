	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    state.tmp.push(tmp2);
				    state.memory.write(toWord(tmp1, tmp2),
						       lsb(state.registers.sp));
			    });
	state.cpuQueue.push([](State& state)
			    {
				    Byte tmp2 = state.tmp.top();
				    ...
				    Byte tmp1 = state.tmp.top();
				    state.tmp.pop();
				    state.memory.write(toWord(tmp1, tmp2) + 1,
						       msb(state.registers.sp));
			    });
}
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (((state.registers.h & 0xf) + (state.registers.b & 0xf) + (carry & 0xf)) & 0x10)
					    state.registers.setHalfCarryFlag();
				    if (((state.registers.h & 0xff) + (state.registers.b & 0xff) + (carry & 0xff)) &
					0x100)
					    state.registers.setCarryFlag();
...
void Instructions::JR_r8(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.registers.pc += static_cast<SignedByte>(state.memory.read(
					    state.registers.pc++));
			    });
	state.cpuQueue.push([](State& state)
			    {
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (((state.registers.h & 0xf) + (state.registers.d & 0xf) + (carry & 0xf)) & 0x10)
					    state.registers.setHalfCarryFlag();
				    if (((state.registers.h & 0xff) + (state.registers.d & 0xff) + (carry & 0xff)) &
					0x100)
					    state.registers.setCarryFlag();
...
void Instructions::JR_NZ_r8(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    SignedByte e = static_cast<SignedByte>(state.memory.read(
					    state.registers.pc++));
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (((state.registers.h & 0xf) + (state.registers.h & 0xf) + (carry & 0xf)) & 0x10)
					    state.registers.setHalfCarryFlag();
				    if (((state.registers.h & 0xff) + (state.registers.h & 0xff) + (carry & 0xff)) &
					0x100)
					    state.registers.setCarryFlag();
...
void Instructions::LD_SP_d16(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.registers.sp = toWord(state.tmp.top(),
								state.memory.read(state.registers.pc++));
				    state.tmp.pop();
			    });
}
...
void Instructions::INC_MEM_AT_HL(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.tmp.push(state.memory.read(state.registers.hl));
			    });
	state.cpuQueue.push([](State& state)
			    {
				    state.registers.resetZeroFlag();
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (((state.registers.h & 0xf) + (s & 0xf) + (carry & 0xf)) & 0x10)
					    state.registers.setHalfCarryFlag();
				    if (((state.registers.h & 0xff) + (s & 0xff) + (carry & 0xff)) &
					0x100)
					    state.registers.setCarryFlag();
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    else
				    {
					    if (!(IE & IF & 0x1f)) // no interrupt pending
						    ...
						    state.cpuState = CPUState::HALT_BUG;
				    }
			    });
}
...
void Instructions::ADD_A_MEM_AT_HL(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.registers.clearFlags();
				    if (((state.registers.a & 0xf) + (state.memory.read(state.registers.hl) & 0xf)) &
					0x10)
					    state.registers.setHalfCarryFlag();
				    if (((state.registers.a & 0xff) + (state.memory.read(state.registers.hl) & 0xff)) &
					0x100)
					    state.registers.setCarryFlag();
				    state.registers.a += state.memory.read(state.registers.hl);
				    if (state.registers.a == 0)
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (((state.registers.a & 0xf) + (state.registers.b & 0xf) + (carry & 0xf)) & 0x10)
					    state.registers.setHalfCarryFlag();
				    if (((state.registers.a & 0xff) + (state.registers.b & 0xff) + (carry & 0xff)) &
					0x100)
					    state.registers.setCarryFlag();
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (((state.registers.a & 0xf) + (state.registers.c & 0xf) + (carry & 0xf)) & 0x10)
					    state.registers.setHalfCarryFlag();
				    if (((state.registers.a & 0xff) + (state.registers.c & 0xff) + (carry & 0xff)) &
					0x100)
					    state.registers.setCarryFlag();
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (((state.registers.a & 0xf) + (state.registers.d & 0xf) + (carry & 0xf)) & 0x10)
					    state.registers.setHalfCarryFlag();
				    if (((state.registers.a & 0xff) + (state.registers.d & 0xff) + (carry & 0xff)) &
					0x100)
					    state.registers.setCarryFlag();
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (((state.registers.a & 0xf) + (state.registers.e & 0xf) + (carry & 0xf)) & 0x10)
					    state.registers.setHalfCarryFlag();
				    if (((state.registers.a & 0xff) + (state.registers.e & 0xff) + (carry & 0xff)) &
					0x100)
					    state.registers.setCarryFlag();
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (((state.registers.a & 0xf) + (state.registers.h & 0xf) + (carry & 0xf)) & 0x10)
					    state.registers.setHalfCarryFlag();
				    if (((state.registers.a & 0xff) + (state.registers.h & 0xff) + (carry & 0xff)) &
					0x100)
					    state.registers.setCarryFlag();
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (((state.registers.a & 0xf) + (state.registers.l & 0xf) + (carry & 0xf)) & 0x10)
					    state.registers.setHalfCarryFlag();
				    if (((state.registers.a & 0xff) + (state.registers.l & 0xff) + (carry & 0xff)) &
					0x100)
					    state.registers.setCarryFlag();
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (((state.registers.a & 0xf) + (state.registers.a & 0xf) + (carry & 0xf)) & 0x10)
					    state.registers.setHalfCarryFlag();
				    if (((state.registers.a & 0xff) + (state.registers.a & 0xff) + (carry & 0xff)) &
					0x100)
					    state.registers.setCarryFlag();
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (!state.registers.getZeroFlag())
				    {
					    state.cpuQueue.push([](State& state)
								{
									state.tmp.push(state.memory.read(
										state.registers.sp++));
								});
					    state.cpuQueue.push([](State& state)
								{
									state.registers.pc = toWord(state.tmp.top(),
												    state.memory.read(
													    state.registers.sp++));
									state.tmp.pop();
								});
...
void Instructions::POP_BC(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.tmp.push(state.memory.read(state.registers.sp++));
			    });
	state.cpuQueue.push([](State& state)
			    {
				    state.registers.bc = toWord(state.tmp.top(),
								state.memory.read(state.registers.sp++));
				    state.tmp.pop();
			    });
}
	...
	state.cpuQueue.push([](State& state)
			    {
				    ...
				    if (!state.registers.getZeroFlag())
				    {
					    ...
					    state.cpuQueue.push([](State& state)
								{
									state.memory.write(--state.registers.sp,
											   msb(state.registers.pc));
								});
					    state.cpuQueue.push([](State& state)
								{
									state.memory.write(--state.registers.sp,
											   lsb(state.registers.pc));
									Byte tmp2 = state.tmp.top();
									state.tmp.pop();
									Byte tmp1 = state.tmp.top();
									state.tmp.pop();
...
void Instructions::PUSH_BC(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.memory.write(--state.registers.sp,
						       msb(state.registers.bc));
			    });
	state.cpuQueue.push([](State& state)
			    {
				    state.memory.write(--state.registers.sp,
						       lsb(state.registers.bc));
			    });
}
...
void Instructions::POP_DE(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.registers.de = toWord(state.tmp.top(),
								state.memory.read(state.registers.sp++));
				    state.tmp.pop();
			    });
}
...
void Instructions::PUSH_DE(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.memory.write(--state.registers.sp,
						       msb(state.registers.de));
			    });
	state.cpuQueue.push([](State& state)
			    {
				    state.memory.write(--state.registers.sp,
						       lsb(state.registers.de));
			    });
}
...
void Instructions::POP_HL(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.registers.hl = toWord(state.tmp.top(),
								state.memory.read(state.registers.sp++));
				    state.tmp.pop();
			    });
}
...
void Instructions::PUSH_HL(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.memory.write(--state.registers.sp,
						       msb(state.registers.hl));
			    });
	state.cpuQueue.push([](State& state)
			    {
				    state.memory.write(--state.registers.sp,
						       lsb(state.registers.hl));
			    });
}
...
void Instructions::ADD_SP_r8(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.tmp.push(static_cast<SignedByte>(state.memory.read(state.registers.pc++)));
			    });
	state.cpuQueue.push([](State& state)
			    {
				    state.registers.clearFlags();
...
void Instructions::POP_AF(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.registers.af = toWord(state.tmp.top(),
								state.memory.read(state.registers.sp++));
				    state.tmp.pop();
			    });
}
...
void Instructions::PUSH_AF(State& state)
{
	...
	state.cpuQueue.push([](State& state)
			    {
				    state.memory.write(--state.registers.sp,
						       msb(state.registers.af));
			    });
	state.cpuQueue.push([](State& state)
			    {
				    state.memory.write(--state.registers.sp,
						       lsb(state.registers.af));
			    });
}
...
void Instructions::LD_SP_HL(State& state)
{
	state.cpuQueue.push([](State& state)
			    {
				    state.registers.sp = (state.registers.sp & 0xff00) | (state.registers.hl & 0xff);
			    });
	state.cpuQueue.push([](State& state)
			    {
				    state.registers.sp = (state.registers.sp & 0xff) | (state.registers.hl & 0xff00);
			    });
}
